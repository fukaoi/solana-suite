...
(function (StorageArweave) {
    ...
    const createMetadata = (name, description, imagePath) => {
        let image = path_1.default.basename(imagePath);
        if (isJpegFile(image)) {
    ...
    StorageArweave.upload = (payerSecret, name, description, imagePath) => __awaiter(this, void 0, void 0, function* () {
        const payer = util_1.Util.createKeypair(payerSecret);
        ...
        yield sol_native_1.SolNative.transfer(payer.publicKey.toString(), [payerSecret], constants_1.Constants.AR_SOL_HOLDER_ID, yield calculateArFee(fileBuffers))();
