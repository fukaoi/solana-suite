...
// src/find.ts
import { SplToken as SplToken2 } from "@solana-suite/core";
...
var MintInstruction = class _MintInstruction extends Instruction2 {
  ...
  submit = async () => {
    return Try2(async () => {
      ...
      if (Node.getConnection().rpcEndpoint === Constants22.EndPointUrl.prd) {
        debugLog22("# Change metaplex cluster on mainnet-beta");
        Node.changeConnection({ cluster: Constants22.Cluster.prdMetaplex });
      }
...
((Constants232) => {
  ...
  Constants232.WRAPPED_TOKEN_PROGRAM_ID = new PublicKey3(
    "So11111111111111111111111111111111111111112"
  );
  Constants232.MEMO_PROGRAM_ID = new PublicKey3(
    "Memo1UhkJRfHyvLMcVucJwxXeuD728EqVDDwQDxFMNo"
  );
  Constants232.METAPLEX_PROGRAM_ID = new PublicKey3(
    "metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s"
  );
...
var Constants7;
((Constants223) => {
  Constants223.currentCluster = Config7.cluster.type;
  Constants223.customClusterUrl = Config7.cluster.customClusterUrl;
  Constants223.isDebugging = Config7.debugging;
  Constants223.nftStorageApiKey = Config7.nftstorage.apikey;
  let Cluster;
  ((Cluster2) => {
    Cluster2["prd"] = "mainnet-beta";
    Cluster2["prdMetaplex"] = "mainnet-beta-metaplex";
  ...
  Constants223.WRAPPED_TOKEN_PROGRAM_ID = new PublicKey9(
    "So11111111111111111111111111111111111111112"
  );
  Constants223.MEMO_PROGRAM_ID = new PublicKey9(
    "Memo1UhkJRfHyvLMcVucJwxXeuD728EqVDDwQDxFMNo"
  );
  Constants223.METAPLEX_PROGRAM_ID = new PublicKey9(
    "metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s"
  );
  Constants223.COMMITMENT = "confirmed";
  Constants223.NFT_STORAGE_API_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweERGMjcyN2VkODZhRGU1RTMyZDZDZEJlODc0YzRFNDlEODY1OWZmOEMiLCJpc3MiOiJuZnQtc3RvcmFnZSIsImlhdCI6MTYyMDI2NDk0MzcwNiwibmFtZSI6ImRlbW8ifQ.d4J70mikxRB8a5vwNu6SO5HDA8JaueuseAj7Q_ytMCE";
  Constants223.NFT_STORAGE_GATEWAY_URL = "https://ipfs.io/ipfs";
  Constants223.BUNDLR_NETWORK_URL = (0, Constants223.switchBundlr)(Config7.cluster.type);
...
var AbstractResult7 = class {
  ...
  unwrap(ok, err) {
    const r = this._chain(
      (value) => Result8.ok(ok ? ok(value) : value),
      (error) => err ? Result8.ok(err(error)) : Result8.err(error)
    );
    if (r.isErr) {
      throw r.error;
    }
  ...
  map(ok, err) {
    return this._chain(
      (value) => Result8.ok(ok(value)),
      (error) => Result8.err(err ? err(error) : error)
    );
  }
  chain(ok, err) {
    return this._chain(ok, err || ((error) => Result8.err(error)));
  }
  match(ok, err) {
    this._chain(
      (value) => Result8.ok(ok(value)),
      (error) => Result8.err(err(error))
    );
  }
  /// submit (alias Instruction.submit) ////
  async submit() {
    try {
      ...
      return Result8.err(Error("Only Instruction object"));
    } catch (err) {
      return Result8.err(err);
    }
  }
};
...
((TraditionalNft10) => {
  TraditionalNft10.createDeleagateInstruction = (mint2, owner, delegateAuthority) => {
    const tokenAccount = getAssociatedTokenAddressSync3(mint2, owner);
  ...
  TraditionalNft10.createMintInstructions = async (mint2, owner, nftMetadata, feePayer, isMutable) => {
    const ata = getAssociatedTokenAddressSync3(mint2, owner);
