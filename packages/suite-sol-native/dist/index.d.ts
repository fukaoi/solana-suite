...
declare const SolNative: {
    transferWithMultisig: (owner: Pubkey, dest: Pubkey, signers: Secret$1[], amount: number, options?: Partial<AuthorityOptions>) => Promise<Result<CommonStructure, Error>>;
    transfer: (source: Pubkey, dest: Pubkey, signers: Secret$1[], amount: number, options?: Partial<AuthorityOptions>) => Result<CommonStructure, Error>;
    gasLessTransfer: (owner: Pubkey, dest: Pubkey, signers: Secret$1[], amount: number, feePayer: Pubkey) => Promise<Result<PartialSignStructure, Error>>;
    findByOwner: (owner: Pubkey) => Promise<Result<OwnerInfo, Error>>;
};

export { Account, FilterOptions, FilterType, KeypairAccount, Memo, MintTo, MintToChecked, ModuleName, Node, OwnerInfo, PostTokenAccount, Pubkey, Secret$1 as Secret, SolNative, Transfer, TransferChecked, Validator, ValidatorError, WithMemo };
