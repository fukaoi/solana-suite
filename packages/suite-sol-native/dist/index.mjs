...
((SolNative6) => {
  const RADIX = 10;
  SolNative6.feePayerPartialSignTransfer = async (owner, dest, signers, amount, feePayer) => {
    return Try(async () => {
      const blockHashObj = await Node.getConnection().getLatestBlockhash();
...
((SolNative6) => {
  const RADIX = 10;
  SolNative6.transfer = (source, dest, signers, amount, feePayer) => {
    return Try(() => {
...
((SolNative6) => {
  const RADIX = 10;
  SolNative6.transferWithMultisig = async (owner, dest, signers, amount, feePayer) => {
    return Try(async () => {
