...
((SolNative6) => {
  ...
  SolNative6.transferWithMultisig = async (owner, dest, multisig, amount, options = {}) => {
    return Try(async () => {
      ...
      const wrapped = await (0, import_spl_token.createWrappedNativeAccount)(
        connection,
        payer.toKeypair(),
        owner.toPublicKey(),
      ...
      const instructions = [];
      const token = await (0, import_spl_token.createMint)(
        connection,
        payer.toKeypair(),
        owner.toPublicKey(),
        owner.toPublicKey(),
