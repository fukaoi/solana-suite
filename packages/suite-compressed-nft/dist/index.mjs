  ...
  CompressedNft8.initTree = (feePayer, maxDepth = 14, maxBufferSize = 64) => {
    return Try(async () => {
      ...
      const inst1 = SystemProgram.createAccount({
        ...
        space,
        programId: SPL_ACCOUNT_COMPRESSION_PROGRAM_ID2
      });
...
((CompressedNft8) => {
  ...
  CompressedNft8.mint = async (owner, signer, input, treeOwner, collectionMint, options = {}) => {
    return Try(async () => {
      ...
      let converted = Converter4.CompressedNftMetadata.intoInfra(
        input,
        uri,
        sellerFeeBasisPoints
      );
...
var RegularNft5;
((RegularNft11) => {
  RegularNft11.gasLessMint = async (owner, signer, input, feePayer, freezeAuthority) => {
    return Try(async () => {
      const valid = Validator.checkAll(input);
      if (valid.isErr) {
        throw valid.error;
...
((RegularNft11) => {
  RegularNft11.mintCollection = (owner, signer, input, feePayer, freezeAuthority, collectionSize = 0) => {
    return Try(async () => {
      ...
      let datav2 = Converter4.RegularNftMetadata.intoInfra(input, uri, 0);
      const isMutable = input.isMutable === void 0 ? true : input.isMutable;
      debugLog("# input: ", input);
      debugLog("# datav2: ", datav2);
...
export {
  Account,
  CompressedNft7 as CompressedNft,
  FilterOptions,
  FilterType,
  ModuleName,
  Node,
