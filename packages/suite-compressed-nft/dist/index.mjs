...
var CompressedNft;
((CompressedNft7) => {
  CompressedNft7.fetchOffchain = async (uri) => {
    const json = await (await fetch(uri)).json();
    return json;
  };
  CompressedNft7.findByOwner = async (owner, limit = 1e3, page = 1, sortBy, before, after) => {
    return Try(async () => {
      ...
      return await Promise.all(
        assets.value.items.map(async (item) => {
          const offchain = await (0, CompressedNft7.fetchOffchain)(item.content.json_uri);
          console.log(item);
          const merged = { ...item, ...offchain };
          return Converter5.CompressedNftMetadata.intoUser(merged);
        })
      );
    });
  };
...
((CompressedNft7) => {
  ...
  CompressedNft7.Tree = Tree;
  CompressedNft7.initTree = (feePayer, maxDepth = 14, maxBufferSize = 64) => {
    return Try(async () => {
...
var CompressedNft3;
((CompressedNft7) => {
  CompressedNft7.createVerifyCreatorsInstruction = async (creators, assetId, treeOwner, metadata, feePayer) => {
  ...
  CompressedNft7.createDeleagateInstruction = async (assetId) => {
    let rpcAssetProof = await DasApi.getAssetProof(assetId.toString());
    const rpcAsset = await DasApi.getAsset(assetId.toString());
    if (rpcAssetProof.isErr || rpcAsset.isErr) {
      throw Error("Rise error when get asset proof or asset");
  ...
  CompressedNft7.mint = async (owner, signer, input, treeOwner, collectionMint, feePayer, receiver) => {
    return Try(async () => {
      ...
      if (input.properties && input.storageType) {
        properties = await Converter5.Properties.intoInfra(
          input.properties,
          Storage.uploadFile,
          input.storageType,
          payer
      ...
      const sellerFeeBasisPoints = Converter5.Royalty.intoInfra(input.royalty);
      ...
      let converted = Converter5.CompressedNftMetadata.intoInfra(
        input,
        uri,
        sellerFeeBasisPoints
      );
      ...
      return new MintTransaction(
        ...
        [signer.toKeypair()],
        payer.toKeypair(),
        new CompressedNft2.Tree(treeOwner)
      );
    });
  };
...
import { Metadata, TokenStandard as TokenStandard2 } from "@metaplex-foundation/mpl-token-metadata";
...
((SplToken11) => {
  ...
  const converter = (tokenStandard, metadata, json, tokenAmount) => {
    if (tokenStandard === TokenStandard2.Fungible) {
      return Converter5.TokenMetadata.intoUser(
        {
          onchain: metadata,
          offchain: json
        },
      ...
      return Converter5.RegularNftMetadata.intoUser(
        {
          onchain: metadata,
          offchain: json
        },
...
((SplToken11) => {
  ...
  SplToken11.mint = async (owner, signer, totalAmount, mintDecimal, input, feePayer, freezeAuthority) => {
    return Try(async () => {
      ...
      const datav2 = Converter5.TokenMetadata.intoInfra(
        input,
        uri,
        sellerFeeBasisPoints
      );
...
import {
  TokenStandard as TokenStandard3
} from "@metaplex-foundation/mpl-token-metadata";
var RegularNft2;
((RegularNft11) => {
...
((RegularNft11) => {
  ...
  RegularNft11.mint = async (owner, signer, input, feePayer, freezeAuthority) => {
    return Try(async () => {
      ...
      let datav2 = Converter5.RegularNftMetadata.intoInfra(
        input,
        uri,
        sellerFeeBasisPoints
      );
  ...
  RegularNft11.feePayerPartialSignMint = async (owner, signer, input, feePayer, freezeAuthority) => {
    return Try(async () => {
      ...
      if (input.filePath && input.storageType === "nftStorage") {
        const properties = await Converter5.Properties.intoInfra(
          input.properties,
          Storage.uploadFile,
          input.storageType
        );
      ...
      let datav2 = Converter5.NftMetadata.intoInfra(
        input,
        uri,
        sellerFeeBasisPoints
      );
      ...
      if (input.collection && input.collection) {
        collection = Converter5.Collection.intoInfra(input.collection);
...
((RegularNft11) => {
  RegularNft11.mintCollection = (owner, signer, input, feePayer, freezeAuthority, collectionSize = 0) => {
    return Try(async () => {
      ...
      let datav2 = Converter5.RegularNftMetadata.intoInfra(input, uri, 0);
      const isMutable = input.isMutable === void 0 ? true : input.isMutable;
      debugLog("# input: ", input);
      debugLog("# datav2: ", datav2);
...
var CompressedNft4;
((CompressedNft7) => {
  CompressedNft7.mintCollection = (owner, signer, input, feePayer, freezeAuthority) => {
...
var CompressedNft5;
((CompressedNft7) => {
  CompressedNft7.transfer = async (assetId, owner, dest, signer, feePayer) => {
    return Try(async () => {
      const payer = feePayer ? feePayer : signer;
...
export {
  Account,
  CompressedNft6 as CompressedNft,
  FilterOptions,
  FilterType,
  ModuleName,
  Node,
