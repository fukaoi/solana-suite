...
((CompressedNft10) => {
  CompressedNft10.createDeleagate = async (assetId, newDelegate) => {
    ...
    const treeAuthority = Account3.Pda.getTreeAuthority(assetProof.tree_id);
    ...
    const transfer = await Try(async () => {
      ...
      inst.add(
        await CompressedNft4.createTransfer(
          mint,
          new Account3.Keypair({ secret: owner }).pubkey,
          dest,
          feePayer
        )
      );
...
((CompressedNft10) => {
  class Space {
    ...
    getAssetId = async () => {
      ...
      return Account3.Pda.getAssetId(this.spaceOwner, leafIndex);
    };
  }
  ...
  CompressedNft10.initSpace = (owner, maxDepth, maxBufferSize, canopyDepth, options = {}) => {
    return Try(async () => {
      ...
      const treeOwner = Account3.Keypair.create();
...
((CompressedNft10) => {
  ...
  CompressedNft10.mint = async (owner, input, treeOwner, collectionMint, options = {}) => {
    return Try(async () => {
      ...
      const leafDelegate = delegate ? delegate : new Account3.Keypair({ secret: payer }).pubkey;
      const treeAuthority = Account3.Pda.getTreeAuthority(
      ...
      const bubblegumSigner = Account3.Pda.getBgumSigner();
      let properties;
      if (input.properties) {
