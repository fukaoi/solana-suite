...
((DasApi4) => {
  ...
  DasApi4.findByMint = async (mint, isCompressed) => {
    ...
    if (asset.value.compression.compressed === isCompressed) {
      ...
      return Converter5.Nft.intoUser(merged);
    }
    return {};
  };
    ...
    const metadatas = await Promise.all(
      items.filter((item) => item.compression.compressed === isCompressed).map(async (item) => {
        try {
          ...
          return Converter5.Nft.intoUser({
            onchain: item,
            offchain: {}
          });
        }
  ...
  ((TokenMetadata2) => {
    ...
    TokenMetadata2.intoUser = (output, tokenAmount) => {
      return {
        ...
        uses: Converter11.Uses.intoUserSide(output.onchain.uses),
        dateTime: convertTimestampToDateTime(output.offchain.created_at),
        offchain: output.offchain
      };
    };
