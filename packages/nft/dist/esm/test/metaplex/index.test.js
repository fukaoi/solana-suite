...
import { Account, Multisig, SplToken } from 'solana-suite';
...
describe('Metaplex', () => {
    ...
    it('Mint nft', async () => {
        ...
        const inst = await Metaplex.mint(data, source.toPubkey(), [source.toKeypair()]);
    ...
    it('Mint batched nft', async () => {
        ...
        const inst1 = await Metaplex.mint(data, source.toPubkey(), [source.toKeypair()]);
        const inst2 = await Metaplex.mint(data, source.toPubkey(), [source.toKeypair()]);
    ...
    it('Transfer nft', async () => {
        ...
        const inst2 = await SplToken.transferNft(inst1.unwrap().data.toPubkey(), source.toPubkey(), dest.toPubkey(), [source.toKeypair()]);
    ...
    it('Transfer nft with fee payer', async () => {
        ...
        const inst2 = await SplToken.transferNft(inst1.unwrap().data.toPubkey(), source.toPubkey(), dest.toPubkey(), [source.toKeypair()], feePayer.toKeypair());
        const res = await inst2.submit();
        console.log('# signature: ', res.unwrap());
    ...
    it('Transfer nft with multi sig', async () => {
        ...
        const inst2 = await SplToken.transferNft(inst1.unwrap().data.toPubkey(), source.toPubkey(), multisigAddress.toPubkey(), [
            source.toKeypair(),
        ]);
        const resTransfer = await inst2.submit();
        console.log('# signature: ', resTransfer.unwrap());
        // transfer from multisig address to dest
        const inst3 = await SplToken.transferNft(inst1.unwrap().data.toPubkey(), multisigAddress.toPubkey(), dest.toPubkey(), [
            signer1.toKeypair(),
            signer2.toKeypair(),
        ], source.toKeypair());
