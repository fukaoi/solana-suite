...
import { MetaplexMetaData, MetaplexInstructure } from '../../src/metaplex';
...
describe('MetaplexMetaData', () => {
    ...
    it('Create instructions for create metadata', async () => {
        ...
        const res = await MetaplexMetaData.create(data, tokenKey, sourceStr, sourceStr, sourceStr);
        assert.isTrue(res.isOk);
        assert.isArray(res.unwrap());
        assert.isObject(res.unwrap()[0]);
    });
    it('Create instructions for update metadata', async () => {
        ...
        const tokenKey = 'ZSMBYfbdn9eFJxs91p61nMbdZ7JALuXvUukqZu18skM'.toPubkey();
        const res = await MetaplexMetaData.update(data, undefined, undefined, tokenKey, source.toPubkey(), [source.toKeypair()]);
        assert.isTrue(res.isOk);
        assert.isArray(res.unwrap());
        assert.isObject(res.unwrap()[0]);
    });
    it('Get metadata by tokenKey', async () => {
        ...
        const res = await MetaplexMetaData.getByTokenKey('Hn1DMeFF9baMuGVaC5dWhKC2jaPEQnB4pdY9iqz6G4zf'.toPubkey());
        assert.isTrue(res.isOk);
    ...
    it('Get metadata of nft by owner', async () => {
        const res = await MetaplexMetaData.getByOwner('78DybLoke46TR6RW1HWZBMYt7qouGggQJjLATsfL7RwA'.toPubkey());
        assert.isTrue(res.isOk);
        console.log(res);
    });
});
