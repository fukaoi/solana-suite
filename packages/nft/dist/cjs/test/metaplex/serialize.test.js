...
(0, mocha_1.describe)('MetaplexSerialize', () => {
    (0, mocha_1.it)('Decode serialized data', () => {
        const orgData = {
        ...
        const res = metaplex_1.MetaplexSerialize.decode(bufferData);
        if (!res)
            chai_1.assert.fail('None res data');
        console.log('# decode metadata: ', res);
        chai_1.assert.equal(res.name, orgData.name);
        chai_1.assert.equal(res.symbol, orgData.symbol);
        chai_1.assert.equal(res.uri, orgData.uri);
        chai_1.assert.equal(res.update_authority, orgData.updateAuthority);
        chai_1.assert.equal(res.seller_fee_basis_points, orgData.sellerFeeBasisPoints);
    });
});
