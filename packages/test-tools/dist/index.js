...
((Account5) => {
  ...
  ((Associated2) => {
    Associated2.makeOrCreateInstruction = async (
      ...
      const associatedTokenAccount = (0,
      import_spl_token.getAssociatedTokenAddressSync)(
        mint.toPublicKey(),
        owner.toPublicKey(),
        allowOwnerOffCurve,
      ...
      try {
        ...
        if (
          !(error instanceof import_spl_token.TokenAccountNotFoundError) &&
          !(error instanceof import_spl_token.TokenInvalidAccountOwnerError)
        ) {
          throw Error('Unexpected error');
        }
        const payer = !feePayer ? owner : feePayer;
        const inst = (0,
        import_spl_token.createAssociatedTokenAccountInstruction)(
          payer.toPublicKey(),
          associatedTokenAccount,
          owner.toPublicKey(),
          mint.toPublicKey(),
...
Number.prototype.toSol = function () {
  return (0, import_bignumber.BigNumber)(this)
    .div(import_web35.LAMPORTS_PER_SOL)
    .toNumber();
};
Number.prototype.toLamports = function () {
  return (0, import_bignumber.BigNumber)(this)
    .times(import_web35.LAMPORTS_PER_SOL)
    .toNumber();
};
...
((TransactionBuilder10) => {
  ...
  ((Retry2) => {
    ...
    Retry2.isComputeBudgetError = (error) => {
      if (
        typeof error === 'object' &&
        error instanceof import_web38.SendTransactionError
      ) {
  ...
  ((CompressedNftMetadata2) => {
    CompressedNftMetadata2.intoInfra = (input, uri, sellerFeeBasisPoints) => {
      return {
        ...
        editionNonce: 0,
        tokenStandard:
          import_mpl_bubblegum_instructions2.TokenStandard.NonFungible,
        tokenProgramVersion:
          import_mpl_bubblegum_instructions2.TokenProgramVersion.Original,
      };
    };
  })(
...
var requestSol = async (pubkey, sol = 0.1) => {
  let bufferStr = '';
  try {
    bufferStr = import_fs.default.readFileSync(
      `./${LOCAL_KEYPAIR_FILE}`,
      'utf8',
    );
  } catch (_) {
