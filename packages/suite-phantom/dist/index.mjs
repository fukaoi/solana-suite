...
((RegularNft12) => {
  ...
  RegularNft12.burn = (mint, owner, ownerOrMultisig, options = {}) => {
...
var RegularNft2;
((RegularNft12) => {
  RegularNft12.findByOwner = async (owner, options = {}) => {
    return Try(async () => {
      return await DasApi3.findByOwner(owner, false, options);
    });
  };
  RegularNft12.findByMint = async (mint) => {
    return Try(async () => {
      return await DasApi3.findByMint(mint, false);
    });
  };
  RegularNft12.findByCollection = async (collectionMint, options = {}) => {
    return Try(async () => {
...
var RegularNft3;
((RegularNft12) => {
  RegularNft12.freeze = (mint, owner, freezeAuthority, options = {}) => {
    return Try(() => {
      const payer = options.feePayer ? options.feePayer : freezeAuthority;
      ...
      const editionAddress = Account3.Pda.getMasterEdition(mint);
      const inst = createFreezeDelegatedAccountInstruction({
        delegate: new Account3.Keypair({
...
((RegularNft12) => {
  const NFT_AMOUNT = 1;
  const DEFAULT_STORAGE_TYPE = "nftStorage";
  RegularNft12.createVerifyCreator = (mint2, creator) => {
    const metadata = Account3.Pda.getMetadata(mint2.toString());
    return createSignMetadataInstruction({
      metadata,
      creator
    });
  };
  RegularNft12.createDeleagate = (mint2, owner, delegateAuthority) => {
    const tokenAccount = getAssociatedTokenAddressSync6(mint2, owner);
  ...
  RegularNft12.createVerifySizedCollection = (collectionChild, collectionParent, feePayer) => {
    const collectionMetadata = Account3.Pda.getMetadata(
      collectionParent.toString()
    );
    const collectionMasterEditionAccount = Account3.Pda.getMasterEdition(
      collectionParent.toString()
    );
    return createVerifySizedCollectionItemInstruction({
      ...
      metadata: Account3.Pda.getMetadata(collectionChild.toString()),
      payer: feePayer,
      collectionAuthority: feePayer
    });
  };
  RegularNft12.createMint = async (mint2, owner, nftMetadata, feePayer, isMutable) => {
    const ata = getAssociatedTokenAddressSync6(mint2, owner);
    const tokenMetadataPubkey = Account3.Pda.getMetadata(mint2.toString());
    const masterEditionPubkey = Account3.Pda.getMasterEdition(mint2.toString());
    const connection = Node.getConnection();
    const instructions = [];
    instructions.push(
    ...
    instructions.push(
      createAssociatedTokenAccountInstruction2(feePayer, ata, owner, mint2)
    );
  ...
  RegularNft12.mint = async (owner, input, options = {}) => {
    return Try(async () => {
      const valid = Validator.checkAll(input);
      if (valid.isErr) {
        throw valid.error;
      ...
      debugLog("# datav2: ", datav2);
      const mint2 = Account3.Keypair.create();
      const instructions = await (0, RegularNft12.createMint)(
        mint2.toPublicKey(),
        ownerPublicKey,
        datav2,
        payer.toKeypair().publicKey,
      ...
      if (freezeAuthority) {
        instructions.push(
          (0, RegularNft12.createDeleagate)(
            mint2.toPublicKey(),
            ownerPublicKey,
            freezeAuthority.toPublicKey()
          )
      ...
      if (input.collection) {
        instructions.push(
          (0, RegularNft12.createVerifySizedCollection)(
            mint2.toPublicKey(),
      ...
      if (input.creators) {
        input.creators.forEach((creator) => {
          if (Account3.Keypair.isSecret(creator.secret)) {
            const creatorPubkey = creator.address.toPublicKey();
            const inst = (0, RegularNft12.createVerifyCreator)(mint2.toPublicKey(), creatorPubkey);
...
((RegularNft12) => {
  const DEFAULT_STORAGE_TYPE = "nftStorage";
  RegularNft12.gasLessMint = async (owner, input, feePayer, options = {}) => {
    return Try(async () => {
      const valid = Validator.checkAll(input);
      if (valid.isErr) {
        throw valid.error;
      ...
      debugLog("# datav2: ", datav2);
      const mint = Account3.Keypair.create();
...
((RegularNft12) => {
  const NFT_AMOUNT = 1;
  const NFT_DECIMALS = 0;
  RegularNft12.gasLessTransfer = async (mint, owner, dest, feePayer) => {
...
((RegularNft12) => {
  ...
  const DEFAULT_STORAGE_TYPE = "nftStorage";
  RegularNft12.mintCollection = (owner, input, options = {}) => {
    return Try(async () => {
      const valid = Validator.checkAll(input);
      if (valid.isErr) {
        throw valid.error;
      ...
      debugLog("# input: ", input);
      debugLog("# datav2: ", datav2);
      const collectionMint = Account3.Keypair.create();
      const collectionMetadataAccount = Account3.Pda.getMetadata(
...
import { getAssociatedTokenAddressSync as getAssociatedTokenAddressSync7 } from "@solana/spl-token";
import { createThawDelegatedAccountInstruction } from "@metaplex-foundation/mpl-token-metadata";
var RegularNft8;
((RegularNft12) => {
  RegularNft12.thaw = (mint, owner, freezeAuthority, options = {}) => {
    return Try(() => {
      const payer = options.feePayer ? options.feePayer : freezeAuthority;
      const tokenAccount = getAssociatedTokenAddressSync7(
        mint.toPublicKey(),
        owner.toPublicKey()
      );
      const editionAddress = Account3.Pda.getMasterEdition(mint);
...
((RegularNft12) => {
  const NFT_AMOUNT = 1;
  const NFT_DECIMALS = 0;
  RegularNft12.transfer = (mint, owner, dest, ownerOrMultisig, options = {}) => {
...
var RegularNft11 = { ...PhantomMetaplex };
