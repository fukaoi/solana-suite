...
((SplToken11) => {
  ...
  const createFreezeAuthority = (mint2, owner, freezeAuthority) => {
  ...
  const createMint = async (mint2, owner, totalAmount, mintDecimal, tokenMetadata, feePayer, isMutable) => {
    const connection = Node.getConnection();
...
((RegularNft11) => {
  const NFT_AMOUNT = 1;
  const DEFAULT_STORAGE_TYPE = "nftStorage";
  const createVerifyCreator = (mint2, creator) => {
  ...
  const createDeleagate = (mint2, owner, delegateAuthority) => {
  ...
  const createVerifySizedCollection = (collectionChild, collectionParent, feePayer) => {
  ...
  const createMint = async (mint2, owner, nftMetadata, feePayer, isMutable) => {
      ...
      if (input.creators) {
        input.creators.forEach((creator) => {
          if (Account4.Keypair.isSecret(creator.secret)) {
            ...
            const inst = createVerifyCreator(mint2.toPublicKey(), creatorPubkey);
...
import { Keypair as Keypair4, Transaction as Transaction9 } from "@solana/web3.js";
