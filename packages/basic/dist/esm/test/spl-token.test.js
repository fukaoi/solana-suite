...
import { Account, SplToken, Multisig } from '../src/';
let source;
let dest;
let tokenKeyStr;
...
describe('SplToken', () => {
    ...
    it('Create token', async () => {
        ...
        const inst = await SplToken.mint(source.pubkey.toPubkey(), [source.secret.toKeypair()], TOKEN_TOTAL_AMOUNT, MINT_DECIMAL);
    ...
    it('[Err]lack signer for multisig', async () => {
        ...
        const TOKEN_TOTAL_AMOUNT = 10000000;
        const mint = await SplToken.mint(multisig.unwrap().data.toPubkey(), [
            source.toKeypair(),
            signer1.toKeypair(),
    ...
    it('Create token, batch transfer', async () => {
        const TOKEN_TOTAL_AMOUNT = 10000000;
        const inst1 = await SplToken.mint(source.pubkey.toPubkey(), [
            source.secret.toKeypair(),
        ], TOKEN_TOTAL_AMOUNT, MINT_DECIMAL);
        assert.isTrue(inst1.isOk, `${inst1.unwrap()}`);
        const token = inst1.unwrap().data;
        console.log('# tokenKey: ', token);
        const inst2 = await SplToken.transfer(token.toPubkey(), source.pubkey.toPubkey(), dest.pubkey.toPubkey(), [
            source.secret.toKeypair(),
        ], 1, MINT_DECIMAL, source.secret.toKeypair());
        assert.isTrue(inst1.isOk);
        const inst3 = await SplToken.transfer(token.toPubkey(), source.pubkey.toPubkey(), dest.pubkey.toPubkey(), [
            source.secret.toKeypair(),
        ], 1, MINT_DECIMAL, source.secret.toKeypair());
    ...
    it('Create token, transfer with multisig and fee payer', async () => {
        ...
        const signer1 = Account.create();
        const signer2 = Account.create();
        const multiInst = await Multisig.create(2, source.secret.toKeypair(), [
        ...
        // create nft
        const TOKEN_TOTAL_AMOUNT = 10000000;
        const mintInst = await SplToken.mint(multisig, [
            source.secret.toKeypair(),
        ...
        const inst = await SplToken.transfer(token, multisig, dest.pubkey.toPubkey(), [
            signer1.secret.toKeypair(),
            signer2.secret.toKeypair(),
        ], 1, MINT_DECIMAL, source.secret.toKeypair());
        assert.isTrue(inst.isOk, `${inst.unwrap()}`);
    ...
    it('Subscribe a account(pubkey)', async () => {
        const subscribeId = SplToken.subscribeAccount(dest.pubkey.toPubkey(), (v) => {
...
const sendContinuously = async () => {
    const inst = await SplToken.transfer(tokenKeyStr.toPubkey(), source.pubkey.toPubkey(), dest.pubkey.toPubkey(), [source.secret.toKeypair()], 1, MINT_DECIMAL);
