...
describe('Account', () => {
    ...
    it('Get token balance at publicKey', async () => {
        ...
        const res = await Account.getTokenBalance(pubkey.toPubkey(), tokenkey.toPubkey());
        assert.isTrue(res.isOk);
        console.log('# balance token: ', res.unwrap());
    });
    it('Get lamports balance at publicKey', async () => {
        const res = await Account.getBalance(source.toPubkey(), 'lamports');
        assert.isTrue(res.isOk);
        console.log('# balance lamports: ', res.unwrap());
    });
    it('find token address', async () => {
        const res = await Account.findAssocaiatedTokenAddress('D7dKBiFxWKiSSew4fzinML1so4vEaSPmtiKV6qWMDUJJ'.toPubkey(), '5hj62erLKeKSM29C5oZR8TGei7RrMG79voFkiCotRZmS'.toPubkey());
        assert.isTrue(res.isOk);
