...
import { Memo, Account, SolNative } from '../src';
let source;
let dest;
...
describe('Memo', () => {
    ...
    it('create instruction', async () => {
        const res = Memo.create(DUMMY_DATA, [
            source.toPubkey(),
        ], [
            source.toKeypair()
        ]);
    ...
    it('send memo by own', async () => {
        const inst = Memo.create('{"memo": "send memo by own"}', [
            source.toPubkey(),
        ], [
            source.toKeypair()
        ]);
    ...
    it('send memo by owners', async () => {
        ...
        const inst = Memo.create('{"memo": "send memo by owners"}', [
            otherOwner.toPubkey()
        ], [
            source.toKeypair(),
    ...
    it('send memo by owners with fee payer', async () => {
        ...
        const inst = Memo.create('{"memo": "send memo by owners with fee payer"}', [
            owner1.toPubkey(),
            owner2.toPubkey(),
        ], [
            owner1.toKeypair(),
    ...
    it('send memo and sol transfer by owner', async () => {
        const owner1 = Account.create();
        const owner2 = Account.create();
        const inst1 = Memo.create('{"memo": "send memo by owners with fee payer"}', [
            owner1.toPubkey(),
            owner2.toPubkey(),
        ], [
            owner1.toKeypair(),
        ...
        const inst2 = await SolNative.transfer(source.toPubkey(), dest.toPubkey(), [
            source.toKeypair()
        ], 0.00001);
    ...
    `;
        const inst = Memo.create(data500byte, [
            source.toPubkey()
        ], [
            source.toKeypair()
        ]);
    ...
    it('[Err] Over max limit', async () => {
        ...
        const inst = Memo.create(overData, [
            source.toPubkey()
        ], [
            source.toKeypair()
        ]);
